...
public class ActivityKCB extends Activity {
	...
	private void init() {
		...
		alertDialog = new AlertDialog.Builder(context).setTitle("请输入你的教务处密码")
				.setView(edit)
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						handler.post(new Runnable() {
							...
							public void run() {
								// TODO Auto-generated
								// method stub
								alertChecking = new AlertDialog.Builder(context)
										.setMessage("正在检测").create();
								alertChecking.show();
							}
						});
						new Thread() {
							public void run() {
								JSONObject o = UserServer.userCheck(Values.User
										.getCurrentUserFunId(context), edit
	...
	public void ready() {
		init();
		final String passw = Values.User.getFunPasswd(context,
			...
			new Thread() {
				public void run() {
					JSONObject o = UserServer.userCheck(
							Values.User.getCurrentUserFunId(context), passw);
					...
					if (ss != 0x00000012) {
						handler.post(new Runnable() {
							...
							public void run() {
								...
								Values.User.saveFunPasswd(context, Values.User
										.getCurrentUserFunId(context), "");
								alertDialog.show();
						...
						Values.User
								.saveFunPasswd(context, Values.User
										.getCurrentUserFunId(context), passw);
						pw = passw;
						handler.post(new Runnable() {
							public void run() {
								init(inH, inW);
					...
					public void run() {
						...
						if ((pjlist = MRemoteServer.getKCBListFromDB(Values.User
							...
							pjlist = MRemoteServer.getKCBList(
									Values.User
											.getCurrentUserFunId(getApplicationContext()),
									pw, getApplicationContext(), handler);
							if (pjlist == null) {
								finish();
								return;
							}
						...
						handler.post(new Runnable() {
							...
							public void run() {
								...
								for (int i = 1; i < 12; i++) {
									...
									glp.columnSpec = GridLayout.spec(1, 9,
											GridLayout.FILL);
								...
								for (int i = 0; i < pjlist.size(); i++) {
									...
									glp.rowSpec = GridLayout.spec(
											v.getStartTime() + 1, v.getTimes(),
											GridLayout.FILL);
									glp.columnSpec = GridLayout.spec(
											v.getStartWeek() + 1, 1);
									glp.width = (int) weightForInit;
									glp.height = (int) heightForInit;
									// v.setLayoutParams(glp);
									View vv = new View(context);
									vv.setBackgroundColor(Color.rgb(
											v.getColor() / 0x10000,
											v.getColor() / 0x100 - v.getColor()
													/ 0x10000 * 0x100,
											v.getColor() % 0x100));
									gl.addView(vv, glp);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				if ((pjlist = MRemoteServer.getKCBListFromDB(Values.User
					...
					pjlist = MRemoteServer.getKCBList(Values.User
							.getCurrentUserFunId(getApplicationContext()), pw,
				...
				handler.post(new Runnable() {
					...
					public void run() {
						...
						for (int i = 0; i < pjlist.size(); i++) {
							...
							glp = new GridLayout.LayoutParams();
							// glp.columnSpec = GridLayout.spec(i,1);
							glp.rowSpec = GridLayout.spec(v.getStartTime() + 1,
									v.getTimes(), GridLayout.FILL);
							glp.columnSpec = GridLayout.spec(
									v.getStartWeek() + 1, 1);
							glp.width = (int) w;
							glp.height = (int) h;
							...
							Log.i("ss", "" + v.getColor());
							vv.setBackgroundColor(Color.rgb(
									v.getColor() / 0x10000, v.getColor()
											/ 0x100 - v.getColor() / 0x10000
											* 0x100, v.getColor() % 0x100));
							gl.addView(vv, glp);
							gl.addView(v.getView(null), glp);
						}
					}
